...
export const CONTRACT_MULTIPLIERS: Record<ActiveContract, number> = {
  petite: 1,
  garde: 2,
  "garde-sans": 4,
  "garde-contre": 6
...
const thresholdsByOudlers: Record<number, number> = {
  0: 56,
  1: 51,
  2: 41,
  3: 36
...
function getCards(cardsByPlayer: Record<PlayerId, Card[]>, playerId: PlayerId): Card[] {
...
function countOudlers(cards: Card[]): number {
  return cards.filter((card) => card.isOudler).length;
}

export function getThreshold(oudlerCount: number): number {
...
export function getTakerTeam(round: RoundState): PlayerId[] {
  if (!round.takerId) {
...
export function scoreRound(round: RoundState): ContractRoundResult {
  ...
  const deltas = Object.fromEntries(round.playerIds.map((playerId) => [playerId, 0])) as Record<
    PlayerId,
    number
  >;
...
export function scoreTwoPlayerRound(round: RoundState): DuelRoundResult {
